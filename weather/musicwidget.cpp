...
void musicwidget::musicPlayerInit()
{
    ...
    playlist = new QMediaPlaylist(this);//create the music play list
    playlist->setPlaybackMode(QMediaPlaylist::Sequential);//Set playback mode
...
void musicwidget::on_metaDataAvailableChanged(bool available)
{
    ...
    //loading lyric files
    QString lrcPath=music->currentMedia().canonicalUrl().toLocalFile();
...
void musicwidget::on_musicOder_clicked()
{
    ...
    if(oderStatus)
    {
       ...
       playlist->setPlaybackMode(QMediaPlaylist::Sequential);//Set play mode (sequential play)
    }
    else
    {
       oderStyle=QString(" QPushButton {border-image: url(:/image/Group.png);}");
       playlist->setPlaybackMode(QMediaPlaylist::CurrentItemInLoop);//Set playback mode (Single loop)
    }
